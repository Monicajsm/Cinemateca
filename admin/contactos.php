...
<body class="body">	
	...
	<section class="dados-lista">
		...
		<table class="tabela-dados-lista">
			...
			<tbody>
				<?php 
				$pesquisa = $mysqli->query("SELECT * FROM `contactos` ORDER BY `id` DESC");
				if ($pesquisa->num_rows>0) {
					while ($print = $pesquisa->fetch_object()) {
						echo '<tr>
							...
							<td><a href="tel:'.$print->tel.'">'.$print->tel.'</a></td>
							<td>'.$print->data.'</td>
							<td>'.$print->assunto.'</td>
							<td>'.$print->conteudo.'</td>
							<td class="accao">
								<a href="controladores/controller-contacto.php?accao=apagar&id='.$print->id.'" class="del">APAGAR</a></td>
							</tr>
						';
					}
				}
