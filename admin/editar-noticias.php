<?php
	...
	if (isset($_GET['id']) && $_GET['id'] != '') {
		$idNot = $_GET['id'];
		$pesquisa = $mysqli->query("SELECT * FROM `noticias` WHERE `id` = '$idNot'");
	...
	if(isSet($_POST['enviar']) && $_POST['enviar']!=''){
		...
		if (!empty($titulo) && !empty($conteudo)) {
			...
			$actualizar = $mysqli->query("UPDATE `noticias` SET
				`titulo` = '$titulo',
				`conteudo` = '$conteudo',
				`img` = '$img'
				WHERE
			...
			if ($actualizar) {
				$erro = '<div class="alert alert-success">Noticia inserida com sucesso!</div>';
...
<body class="body">	
	...
	<section class="cabecalho">
		<h1 class="titulo-crm">EDITAR NOTICIA</h1>
		<h6 class="descricao-crm">Aqui editamos as noticias do site.</h6>
	</section>
			...
			<form method="POST" action=""  enctype="multipart/form-data">
				...
				<div class="row-no-padding">
					...
					<div class="col-md-6">
						<label>
							CONTEUDO<br>
							<textarea placeholder="CONTEUDO" name="conteudo"><?php echo $print->conteudo; ?></textarea>
						</label>
					</div>
					...
					<div class="col-md-4">
						...
						if ($print->img != '') {
							echo '<div class="bgImg" style="width: 200px;"><img src="../'.$print->img.'"></div>';
						}
