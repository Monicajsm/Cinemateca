<?php
	...
	if (isset($_GET['id']) && $_GET['id'] != '') {
		$idBANNER = $_GET['id'];
		$pesquisa = $mysqli->query("SELECT * FROM `banner_principal` WHERE `id` = '$idBANNER'");
	...
	if(isSet($_POST['enviar']) && $_POST['enviar']!=''){
		...
		if (!empty($aviso) && !empty($titulo)) {
		
			// Query de insercao
			$editar = $mysqli->query("UPDATE `banner_principal` SET 
			`url` = '$img',
			`target` = '$target',
			`titulo` = '$titulo',
			`data` = '$data'
...
<html>
<head>
	<title>Backend - Editar Banner Principal</title>
...
<body class="body">	
	...
	<section class="cabecalho">
		<h1 class="titulo-crm">EDITAR BANNER PRINCIPAL</h1>
		<h6 class="descricao-crm">Aqui editamos o banner principal do site.</h6>
	</section>
	<section class="dados-lista">
		...
		<input type="hidden" name="idbanner" value="<?php echo $idBANNER; ?>">
		<input type="hidden" name="imgAntiga" value="<?php echo $print->img; ?>">
			...
			<div class="row-no-padding">
				...
				<div class="col-md-6">
					...
					<?php
					if ($print->url != '') {
						echo '<img src="../' . $print->url . '"  style="width:200px;">';
					}
					?>
					</label>
				</div>
