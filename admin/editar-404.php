<?php
	...
	if (isset($_GET['id']) && $_GET['id'] != '') {
		$id404 = $_GET['id'];
		$pesquisa = $mysqli->query("SELECT * FROM `404_page` WHERE `id` = '$id404'");
	...
	if(isSet($_POST['enviar']) && $_POST['enviar']!=''){
		...
		if (!empty($aviso) && !empty($titulo)) {
			...
			// Query de insercao
			$editar = $mysqli->query("UPDATE `404_page` SET 
			`img` = '$img',
			`aviso` = '$aviso',
			`titulo` = '$titulo'
			WHERE
...
<html>
<head>
	<title>Backend - Editar Aviso 404</title>
...
<body class="body">	
	...
	<section class="cabecalho">
		<h1 class="titulo-crm">EDITAR AVISO 404</h1>
		<h6 class="descricao-crm">Aqui editamos o aviso de erro 404.</h6>
	</section>
	<section class="dados-lista">
		...
		<input type="hidden" name="iderror" value="<?php echo $id404; ?>">
		<input type="hidden" name="imgAntiga" value="<?php echo $print->img; ?>">
			...
			<div class="row-no-padding">
				...
				<div class="col-md-6">
					<label>AVISO
						<input type="text" name="aviso" value="<?php echo $print->aviso; ?>">	
					</label>
				</div>
				<!----- IMAGEM -------->
				<div class="col-md-6">
